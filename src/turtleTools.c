...
void dialUpdate() {
    for (uint32_t i = 0; i < tt_elements.dials -> length; i++) {
        ...
        turtlePenUp();
        /* mouse */
        if (dialp -> enabled == TT_ELEMENT_ENABLED && (tt_globals.elementLogicTypeOld < TT_ELEMENT_PRIORITY_DIAL || (tt_globals.elementLogicTypeOld == TT_ELEMENT_PRIORITY_DIAL && tt_globals.elementLogicIndexOld <= (int32_t ) i))) {
            if (turtleMouseDown()) {
                if (dialp -> status[0] < 0) {
...
void sliderUpdate() {
    for (uint32_t i = 0; i < tt_elements.sliders -> length; i++) {
        ...
        turtlePenUp();
        /* mouse */
        if (sliderp -> enabled == TT_ELEMENT_ENABLED && (tt_globals.elementLogicTypeOld < TT_ELEMENT_PRIORITY_SLIDER || (tt_globals.elementLogicTypeOld == TT_ELEMENT_PRIORITY_SLIDER && tt_globals.elementLogicIndexOld <= (int32_t ) i))) {
            if (turtleMouseDown()) {
                if (sliderp -> status < 0) {
                    sliderp -> status *= -1;
                }
...
void scrollbarUpdate() {
    for (uint32_t i = 0; i < tt_elements.scrollbars -> length; i++) {
        ...
        if (scrollbarp -> type == TT_SCROLLBAR_HORIZONTAL) {
            ...
            turtlePenUp();
            if (scrollbarp -> enabled == TT_ELEMENT_ENABLED && (tt_globals.elementLogicTypeOld < TT_ELEMENT_PRIORITY_SCROLLBAR || (tt_globals.elementLogicTypeOld == TT_ELEMENT_PRIORITY_SCROLLBAR && tt_globals.elementLogicIndexOld <= (int32_t ) i))) {
...
void dropdownUpdate() {
    for (uint32_t i = 0; i < tt_elements.dropdowns -> length; i++) {
        ...
        int32_t dropdownDirection = dropdownp -> direction; // TODO - handle auto
            ...
            if (dropdownp -> enabled == TT_ELEMENT_ENABLED && tt_ribbon.mainselect[2] == -1) {
                ...
                if (dropdownp -> status == 2 || dropdownp -> status == 1) {
                    ...
                    if (turtle.mouseX > dropdownMaxXFactor[0] && turtle.mouseX < dropdownMaxXFactor[1] && ((directionRender == 1 && turtle.mouseY > dropdownY - dropdownp -> size * 0.9 - (dropdownp -> options -> length - 1) * itemHeight && turtle.mouseY <= dropdownY + dropdownp -> size * 0.9) || (directionRender == -1 && turtle.mouseY < dropdownY + dropdownp -> size * 0.9 + (dropdownp -> options -> length - 1) * itemHeight && turtle.mouseY <= dropdownY - dropdownp -> size * 0.9))) {
                        ...
                        turtleRectangle(dropdownMaxXFactor[0], dropdownY - dropdownp -> size * 0.9 * directionRender - selected * itemHeight * directionRender, dropdownMaxXFactor[1], dropdownY + dropdownp -> size * 0.9 - selected * itemHeight * directionRender);
                    ...
                    for (uint32_t i = 0; i < dropdownp -> options -> length; i++) {
                        if (i != dropdownp -> index) {
                            if (dropdownp -> align == TT_DROPDOWN_ALIGN_LEFT) {
                                turtleTextWriteUnicode(dropdownp -> options -> data[i].s, dropdownMaxXFactor[0] + dropdownp -> size / 5, dropdownY - renderIndex * itemHeight * directionRender, dropdownp -> size - 1, dropdownAlignFactor);
                            } else if (dropdownp -> align == TT_DROPDOWN_ALIGN_CENTER) {
