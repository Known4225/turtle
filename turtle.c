...
int main(int argc, char *argv[]) {
    ...
    while (turtle.popupClose == 0) {
        ...
        turtleTextWriteUnicode((uint8_t *) u8"IÌÍÎÏĪİĮJKĶLĹĽĻŁĿMNŃŇÑŅOÒÓÔÖÕŐØŒPQRŔ", scrollbarVarX * -5 + 320, scrollbarVarY * 3.3 - 195, 10, 0);
        turtleTextWriteUnicode((uint8_t *) u8"ŘSŚŠŞȘẞTŤȚÞUÙÚÛÜŮŰŪŲVWXYÝZŹŽŻaàáâăäã", scrollbarVarX * -5 + 320, scrollbarVarY * 3.3 - 210, 10, 0);
        turtleTextWriteUnicode((uint8_t *) u8"åāąæbcćčċçdďđðeèéěêëēėęəfgğġhħiìíîïī", scrollbarVarX * -5 + 320, scrollbarVarY * 3.3 - 225, 10, 0);
        turtleTextWriteUnicode((uint8_t *) u8"ıįjkķlĺľļłŀmnńňñņoòóôöõőøœpqrŕřsśšşș", scrollbarVarX * -5 + 320, scrollbarVarY * 3.3 - 240, 10, 0);
        turtleTextWriteUnicode((uint8_t *) u8"ßtťțþuùúûüůűūųvwxyýzźžżАБВГҐҒДЂЕЁЄӘЖ", scrollbarVarX * -5 + 320, scrollbarVarY * 3.3 - 255, 10, 0);
        turtleTextWriteUnicode((uint8_t *) u8"ӁЗИӢЙІЇЈКҚҜЛЉМНҢЊОӨПРСТЋУӮҮҰЎФХҲҺЦЧҶ", scrollbarVarX * -5 + 320, scrollbarVarY * 3.3 - 270, 10, 0);
        turtleTextWriteUnicode((uint8_t *) u8"ҸЏШЩЪЫЬЭЮЯабвгґғдђеёєәжӂзиӣйіїјкқҝлљ", scrollbarVarX * -5 + 320, scrollbarVarY * 3.3 - 285, 10, 0);
        turtleTextWriteUnicode((uint8_t *) u8"мнңњоөпрстћуӯүұўфхҳһцчҷҹџшщъыьэюя", scrollbarVarX * -5 + 320, scrollbarVarY * 3.3 - 300, 10, 0);
